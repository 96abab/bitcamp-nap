...
/* [inner] join ~ on 의 문제점
   ...
   => 위 SQL의 실행 결과를 보라
      첨부파일이 없는 2 번은 결과에 포함 X
*/
-- 5) OUTER JOIN
-- 조인 결과에 일치하는 데이터가 없더라도 두 테이블 중에서 한 테이블의 데이터를 
-- 결과로 포함시키는 명령이다
-- 문법
-- select 컬럼명, 컬럼명,....
-- from 테이블1 t1 [left/right] outer join 테이블2 t2 on t1.컬럼=t2컬럼
-- left outer join => 왼쪽 테이블의 데이터는 반드시 포함시키라는 뜻
-- right outer join => 오른쪽 테이블의 데이터는 반드시 포함하라는 뜻
...
-- 2. 컬럼을 나열할 때 한줄에 한 컬럼을 나열한다

select b.no, b.title, b.content, fno, filepath, bno
from board5 b left outer join attach_file5 a on b.no=a.bno
order by no desc;
...
-- 3.강의 정보를 출력할때 센터 이름과 강의실 이름도 함께 출력해 보자
