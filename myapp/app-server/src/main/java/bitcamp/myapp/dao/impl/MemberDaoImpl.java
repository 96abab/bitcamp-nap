...
public class MemberDaoImpl implements MemberDao {
  ...
  public void insert(Member m) {
    try (Statement stmt = con.createStatement()) {
      ...
      stmt.executeUpdate(
          sql,
          Statement.RETURN_GENERATED_KEYS // insert 실행후 자동증다된 PK값을 리턴 받겠다고 선언
          );

      // 1) insert 후에 자동 생성된 PK값을 꺼내올 도구를 준비한다
      try (ResultSet keyRs = stmt.getGeneratedKeys()) {

        // 2) keyRs도구를 이용하여 서버에서 PK값을 가져온다
        keyRs.next();

        // 3) 가져온 PK 레코드에서 PK값을 꺼낸다
        int autoGeneratedMemberId = keyRs.getInt(1); // PK 컬럼에 한개이기 때문에 PK레고드에도 한개의 컬럼만 있다

        // DBMS에서 insert 할때 자동 생성한 member_id값을 Member 객체에 리턴한다
