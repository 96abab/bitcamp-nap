  ...
  public Student[] findAll() {
    try (Connection con = DriverManager.getConnection(
        ...
        ResultSet rs = stmt.executeQuery(
            "select student_id, name,tel,created_date,work,level from app_student order by student_id desc")) {
  ...
  public Student findByNo(int no) {
    try (Connection con = DriverManager.getConnection(
      ...
      while (rs.next()) {
        ...
        s.setTel(rs.getString("tel"));
        s.setCreatedDate(rs.getString("created_date"));
        s.setPostNo(rs.getNString("pst_no"));
        s.setBasicAddress(rs.getString("bas_addr"));
        s.setDetailAddress(rs.getString("bas_addr"));
        s.setWorking(rs.getBoolean("work"));
